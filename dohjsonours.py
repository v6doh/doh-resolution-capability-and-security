...
k_path='/home/ubuntu/tlsgo/aa_2.txt'
def check_doh(ips):
    ip=ips.strip("\n")
    data = {"ip": ip, "flag1": False,"flag2":False}
    #DoH服务器
    url="https://[{}]/dns-query?name=example.com&type=A".format(ip)     # DoH服务器所在url
    ...
    try:
        ...
        with httpx.Client(http1=False,http2=True,verify=False,headers=headers,timeout=5) as client:
            response = client.get(url)
        if response.status_code == 200 and response.headers.get('Content-Type') == 'application/dns-json':
            data["flag1"] = True
